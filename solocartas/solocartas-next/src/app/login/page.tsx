...
function parseLoginError(message: string): string {
  if (message.includes('auth/invalid-credentials') || message.includes('auth/wrong-password')) {
  ...
  if (message.includes('Network') || message.includes('fetch')) {
    return 'Problema de conexión. Revisa tu red e intenta nuevamente.';
  }
    ...
    <div className="min-h-screen flex flex-col items-center justify-center bg-gray-100 py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-md w-full space-y-8 bg-white p-10 rounded-xl shadow-lg">
        ...
        <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
          ...
          {error && (
            <div
              className="bg-red-100 border border-red-400 text-red-700 px-4 py-2 rounded relative text-sm text-center"
              role="alert"
            >
              {error}
            </div>
